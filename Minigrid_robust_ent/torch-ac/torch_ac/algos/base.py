    ...
    def collect_experiences(self):
        ...
        for i in range(self.num_frames_per_proc):
            ...
            for env in self.envs:
                ...
                #concat the agent position with a one hot encoding of the agent's direction:
                pos = np.array(env.agent_pos)
                dirction = env.agent_dir
                dirction_one_hot = np.zeros(4)
                dirction_one_hot[dirction] = 1
                self.agent_positions.append(np.concatenate((pos,dirction_one_hot)))
                
                

            self.agent_poss[i] = self.agent_positions
        ...
        #   - P is self.num_procs,
        #   - D is the dimensionality.s

        exps = DictList()
        exps.obs = [self.obss[i][j]
                    for j in range(self.num_procs)
                    for i in range(self.num_frames_per_proc)]
        exps.agent_pos = [self.agent_poss[i][j]
                          for j in range(self.num_procs)
                          for i in range(self.num_frames_per_proc)]
        ...
        exps.agent_pos = torch.tensor(exps.agent_pos, device=self.device, dtype=torch.float)
