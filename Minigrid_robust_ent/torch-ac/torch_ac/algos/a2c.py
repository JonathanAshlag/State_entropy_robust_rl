        ...
        for i in range(self.recurrence):
            ...
            if self.use_entropy_reward:
                if self.use_value_condition :
                    ...
                    if self.use_true_pos:
                        src_feats = sb.agent_pos
                    else:
                        src_feats = self.random_encoder(sb_obs)
                    if self.use_batch:
                        if self.use_true_pos:
                            tgt_feats = src_feats.clone()
                        else: 
                    ...
                    if self.use_true_pos:
                        s_ent = self.compute_value_condition_state_entropy(src_feats, tgt_feats, value_dist, average_entropy=True)[:,0]
                    else:
                        s_ent = self.compute_value_condition_state_entropy(src_feats[:,:,0,0], tgt_feats,value_dist, average_entropy=True)[:,0]
                    ...
                    if self.use_batch:
                        ...
                        if self.full:
                            ...
                            tgt_feats = torch.tensor(self.replay_buffer[:self.idx], device=self.device)
                    if self.use_true_pos:
                        s_ent = self.compute_state_entropy(src_feats, tgt_feats, average_entropy=True)[:,0]
                    else:
